#!/usr/bin/env python3
"""Scan courseware sources and outputs for unsafe display-fraction shortcuts."""
...
TEXT_EXTENSIONS = {".html", ".htm", ".json", ".js", ".mjs", ".cjs", ".ts", ".tsx", ".jsx", ".md", ".css", ".svg", ".txt"}
DIAGONAL_FRACTIONS = set("¼½¾⅐⅑⅒⅓⅔⅕⅖⅗⅘⅙⅚⅛⅜⅝⅞")
NUMERIC_SLASH = re.compile(r"(?<![\w:/])\d+\s*/\s*\d+(?![\w/])")
FORMULA_CONTEXT = re.compile(r"(?:formula|公式|data-formula|\b[STAF][a-zA-Z]?\s*\(|=)", re.IGNORECASE)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scan formula markup")
...
def main() -> int:
    ...
    report = {
        "validator": "courseware-formula-markup-v1",
