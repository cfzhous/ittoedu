...
取得哈希有效的 `implementation-ready` 交接记录后，当前主干仍必须暂停在实现门禁：`build-project-v7-courseware` 只适用于归档标签 `internal-prototype-1.7.0`，不能生成当前主干只接受的 Project V8。待 Project V8 / Runtime API 2 / Component API 4 完成断代并提供新的权威实现 Skill 后，才可恢复新课例工程生成。涉及编辑器的场景状态、运行时/组件关系、缩略图、编辑/运行画布、Project Schema、组件设计或导出语义时，仍必须完整阅读 [当前创作与接入规范](docs/AI_COURSEWARE_AUTHORING.md)。交接无效、内容缺失、脚本无法追溯或实现 Skill 版本不匹配时不得从聊天或既有实现自行补写，应返回相应编排阶段或保持暂停。
