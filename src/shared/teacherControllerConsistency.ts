...
function isCourseTeacherControllerItem(
  item: LayerItem,
): item is LayerItem & {
  kind: 'native'
  content: { nativeType: 'teacher-controller'; data: { buttons: Array<{ visible: boolean; action: TeacherControllerAction }> } }
} {
...
export function restoreCourseTeacherControllerLayer(entry: ScopedLayerItem): boolean {
  if (!isCourseTeacherControllerItem(entry.item)) return false
