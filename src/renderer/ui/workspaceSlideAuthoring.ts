 ...
 * backend. Runtime/Component author targets follow the same unified-layer path
 * as Native targets, so their canvas edits are allowed in the injected V9
 * backend too; everything else keeps a single-backend decision before it can
 * invoke a mutating command.
 */
export function workspaceAuthoringActionAllowed(
  ...
  return action === 'runtime-edit' || action === 'component-edit'
}
