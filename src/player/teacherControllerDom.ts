...
type DragTarget = 'panel' | 'button' | 'collapse'
...
function pointInRect(x: number, y: number, rect: TeacherControllerRect): boolean {
...
/**
 * Delivery-time DOM teacher controller. It shares the exact layout and session
 * geometry modules with the Phaser renderer, and only adapts pointer/keyboard
 * interaction to the compositor's DOM plane. It owns no course state: session
 * offset/collapse and every action delegate back to the surface host.
 */
export class TeacherControllerDom {
  ...
  #handlePointerDown = (event: PointerEvent): void => {
    ...
    const target = this.#hitTarget(local)
    if (!target) return
    this.#drag = {
  ...
  /** Converts a client point into the frame-local layout coordinate space. */
  #localPoint(event: PointerEvent): { x: number; y: number } {
    const rect = this.#root.getBoundingClientRect()
    const radians = -this.#node.rotation * Math.PI / 180
    const cosine = Math.cos(radians)
    const sine = Math.sin(radians)
    const dx = event.clientX - (rect.left + rect.width / 2)
    const dy = event.clientY - (rect.top + rect.height / 2)
    return {
      x: dx * cosine - dy * sine + this.#node.width / 2,
      y: dx * sine + dy * cosine + this.#node.height / 2,
    }
  }

  /** The only interactive areas: the pill, the buttons, or the draggable panel. */
  #hitTarget(local: { x: number; y: number }): DragTarget | null {
    const layout = this.#layout
    if (layout.collapse && pointInRect(local.x, local.y, layout.collapse)) {
      return 'collapse'
    }
    if (this.#session.collapsed) return null
    for (const button of layout.buttons) {
      if (pointInRect(local.x, local.y, button)) return 'button'
    }
    return 'panel'
  }
  ...
  #buttonActionAt(local: { x: number; y: number }): TeacherControllerAction | undefined {
    return this.#layout.buttons.find((button) => (
      pointInRect(local.x, local.y, button)
    ))?.action
  }
}
