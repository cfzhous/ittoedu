...
const componentManifest: ComponentManifestV3 = {
  schemaVersion: 3,
  runtimeApiVersion: 3,
...
const componentRuntime =
  "window.CoursewareComponent.define({id:'com.example.global-controls',runtimeApiVersion:3,create(){return{destroy(){}}}})"
...
function makeProject(): ProjectDocument {
  return {
    ...
    globalRuntime: {
      ...
      renderMode: 'hybrid',
      source:
        "CoursewareRuntime.define({runtimeApiVersion:1,create(ctx){ctx.dom.overlay.textContent=ctx.content.get('title');return{destroy(){}}}})",
    ...
    scenes: [
      {
        ...
        runtime: {
          ...
          renderMode: 'dom',
          source:
            "CoursewareRuntime.define({runtimeApiVersion:1,create(ctx){ctx.domRoot.textContent=ctx.content.get('hint');return{destroy(){}}}})",
    ...
    componentPackages: {
      'com.example.global-controls@3.0.0': {
        ...
        manifestPath:
          'components/com.example.global-controls@3.0.0/manifest.json',
        runtimePath:
          'components/com.example.global-controls@3.0.0/runtime.js',
      },
    },
    globalLayer: [
      {
...
function makePayload(): ExportPayload {
  ...
  return buildExportPayload({
    ...
    componentPackages: {
      'com.example.global-controls@3.0.0': componentPackage,
    },
  })
}
...
describe('V3 PPTX 静态化', () => {
  ...
  it('缺少实际快照和 fallback 时写入可见警告，不删除可编辑对象', async () => {
    ...
    expect(first).toContain('V3 静态导出警告')
  })
...
describe('V3 PDF 静态化', () => {
  it('播放器快照失败时按 underlay/overlay 合成 fallback，并仅显示可见全局组件占位', async () => {
    ...
    const html = buildPdfPrintHtml('V3 PDF', [pngDataUrl(), pngDataUrl()])
