...
课程专用 Component API 4 DOM 组件，组件 ID 为 `com.alepha.math.motion-function-lab`。它承载旧《让运动变成函数》七幕课的结构化互动与连续联动，不是通用数学模板。整课已被人工拒绝：教学内容规格和呈现脚本不完整、容量与难度不足、组件化范围过大，并存在斜线分数排版错误。后续不得继续扩展本组件来证明新创作工作流有效。
...
稳定阶段由 Project V7 的命名状态承载；组件发出语义事件，场景规则负责进入修复、提示或完成状态。
