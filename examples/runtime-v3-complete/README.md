...
在 Editor 1.6.0 中，选中节点后的“属性/交互”只显示该节点点击规则；右侧常驻“自动化”Tab 显示场景/状态进入、节点激活、动画完成、音视频/组件/运行时事件规则。每个动作步骤可用 `after-previous` / `with-previous` 排列顺序与并行，并设局部延迟。运行时可只负责复杂判定并调用 `ctx.emit()`，再由自动化执行可编辑的入场/退场、状态、媒体或导航动作。
