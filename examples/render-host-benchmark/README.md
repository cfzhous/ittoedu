...
| 05 Component API 3 Phaser 仪表 | V3 兼容组件 | 新架构不要迫使旧 Phaser 组件立即重写 |
...
3. 解析 V4 DOM 和 V3 Phaser manifest，并执行两份组件注册段；
4. 用当前 Project Schema 校验完整 V7 JSON；
5. 生成 `.h5lesson` 后重新解压打开，核对五页、两份 runtime 和两份组件；
6. 使用当前 Player Bundle 生成不依赖网络的单 HTML。

Three.js 的 MIT 许可证和版本信息位于 [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md)。转发生成产物时应同时保留该通知。
...
5. 第五页点击 V3 仪表盘不同位置；确认指针和计数文案更新，且单个旧组件不影响其他场景。
