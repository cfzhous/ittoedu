...
> 状态：**已合入，禁止重做**。夹具 `canvas-runtime.h5lesson` 持久化已切换为 `canvas-runtime`（T1-B）。
...
不要在本任务改 `courseProjectSchema.ts`、删除导入 UI、或重命名 backend。不要开始 P1–P8；若工作区补丁正好覆盖 P 的文件，先提交补丁，在 HANDOFF 写明 P 任务应接在哪次提交之后。
