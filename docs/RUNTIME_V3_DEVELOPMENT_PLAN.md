...
编辑界面明确分流规则：选中节点时，“属性”中的“交互”只管理该节点的 `node.click`；右侧常驻“自动化”Tab 管理 `scene.enter`、`presentation.enter`、`node.activated`、`animation.completed`、音视频事件、`component.event` 和 `runtime.event`。两处共用条件、步骤启动方式、局部延迟和完整动作编辑器。复杂逻辑由运行时/组件完成判定并发出语义事件，自动化负责编排可枚举结果。
...
- 选中节点时，“属性”中的“交互”配置点击映射；常驻“自动化”Tab 配置场景/状态进入、节点激活、动画完成、音视频/组件/运行时事件及顺序/并行动作步骤；
- 通用属性只设置 `playbackInitialVisibility`；动画时机、入场/退场效果、时长、缓动和延迟在动作步骤中编辑，不提供通用关键帧/路径时间轴；
- 媒体面板管理声音导入、删除、声音定义、声道音量和默认静音；删除前检查引用关系；
- 媒体面板列出已有图片/视频素材并可重复添加为新节点；
...
- 编辑器不提供运行时源码编辑器、时间轴或通用节点图；声明式交互映射是受限、可校验的属性式编辑，不等于任意 JavaScript 可视化；
...
内联 Project V7、组件、素材和 Player。保留场景/全局声明式交互、事件驱动入场/退场、声音、视频、场景目录控制器、运行时、组件和状态切换。音视频会转为内联数据；超过 50 MiB 警告，超过 256 MiB 阻止并建议网页包。

### 网页包

分离 `index.html`、播放器、课程数据、工程素材和组件素材。运行语义与单 HTML 相同；资源解析统一适配相对 URL。含大视频的工程应优先选择网页包，避免单 HTML 的体积、加载和内存压力。完整解压后可 `file://` 打开，部署时建议静态服务器。
...
| 元素动画 | `node.enter` / `node.exit`、立即/淡化/四向滑动/缩放、时长/缓动、顺序/并行/延迟、完成事件、中断接管、输入禁用、Player 瞬态可见性与静态稳定帧 |
...
| 编辑易用性 | 图片/视频素材复用、画布 50%–200% 缩放、Ctrl/Command+滚轮、空格/中键平移、复位且不改坐标 |
