...
1. 完整实现教学策划要求的视觉、动画、声音、状态、分支和互动体验；
...
第一优先级永远是成品效果。不得为了组件化、可编辑比例、已有模板或编辑器面板齐全而降低策划要求。
...
编辑器提供“场景 + 场景状态 + 场景/全局声明式交互”创作模型。简单的按钮点击、状态切换、场景跳转、声音播放和静音控制应优先写成 `SceneDocument.interactions` 或课程级 `globalInteractions`：节点基础点击在“属性”中维护，非点击触发在右侧常驻“自动化”Tab 中维护；全局规则用 `scene.in` 限定生效场景。复杂判定、连续拖拽、粒子、时间轴式动画和算法行为仍由运行时或组件承担。AI 仍可在场景运行时、全局运行时和组件代码中使用 Phaser、DOM、SVG、Canvas、WebGL、Tween、程序音效和本地素材。
...
编辑器不负责在界面中创作或修改 JavaScript。运行时源码应由 AI 或可复现生成脚本写入工程；教师主要修改登记后的文案与其他公开属性。
...
不要先制作一份脱离工程规范的网页，再进行第二次接入。正确路径是：

```text
教学策划 + 文案 + 素材
        ↓
逐项设计承载方式、状态、生命周期、编辑入口和验收路径
        ↓
直接生成 Project V7 .h5lesson
...
新创作必须直接写入 `schemaVersion: 7`，并显式包含 `globalLayer`、`globalInteractions`、每个场景的 `interactions`、`media.audio` 和 `playback`。即使暂时没有规则或全局元素，也要保留空数组/空对象及有效默认值。旧 V1–V6 结构只属于加载迁移输入；AI 不得在新工程或生成脚本中继续产出旧字段，也不得通过伪装旧版本来规避 Project V7 校验。
...
Editor 1.6.0 将规则分成两个常驻职责入口：选中节点时，“属性”中的“交互”只维护该节点的 `node.click` 规则；右侧“自动化”Tab 始终可见，维护进入场景/状态、节点激活、动画完成、声音结束、视频生命周期/时间点、组件事件和 `runtime.event` 等非点击规则。进入全局层后，两处改为读写 `globalInteractions`，并提供“生效场景”编辑。运行时事件必须显式选择 `scene` 或 `global` 来源：场景来源校验当前场景 ID，全局来源可由当前场景或全局规则接收。两处共用 `scene.in` / `presentation.in` 条件及元素动画、状态、场景、声音、视频动作编辑器，不得让同一条点击规则同时出现在两个入口。
...
运行时调用 `ctx.emit(eventName, payload)` 时，宿主会附加 `scope` 与场景 ID 并包装为 `runtime:event`。优先让运行时只做复杂判定并发出语义事件，再由右侧“自动化”Tab 按 `scene/global` 来源和事件名执行可编辑的状态、声音、视频或导航动作；不要在运行时里重复硬编码这些稳定结果。组件同样应优先 `emit()` 语义事件，再让 `component.event` 自动化承担可编辑结果映射。
...
- V1/V2 组件继续兼容但只能放在场景；V3 继续使用历史顶层 Phaser 上下文；
...
1. 把教学策划逐项拆成场景/阶段、操作、反馈、结束条件和验收证据。
2. 先确定达到成品效果的最短充分方案，再选择八种承载方式；逐项区分稳定视觉状态、声明式交互与瞬态运行效果。
3. 建立“人工可见文字清单”，为每项指定 `TextNode.text`、运行时 `content.values` 或组件 `props.content`。
4. 为跨场景行为定义稳定场景 ID、事件、`courseState` 键和重播/重开语义。
5. 为每个场景列出基础、初始、缩略图及所有可达稳定状态；把差异写成元素覆盖，而不是运行时重建画面。
...
7. 把可枚举的入场/退场写成自动化动作：明确触发事件、目标元素、顺序/并行关系、局部延迟和完成后的下一步；只有连续路径、关键帧或算法运动进入组件/运行时。
...
不得只写“有互动”“有动画”或“支持编辑”。必须给出可执行的验收路径。
...
- Project V7 Schema、素材引用、场景/全局交互规则、动画动作步骤和组件包校验通过；新工程含 `globalLayer`、`globalInteractions`、每场景 `interactions`、`media.audio` 和 `playback`；V1–V6 可迁移并重新写为 V7；
...
- 场景基础/状态覆盖、原生文字、运行时内容表和 V3/V4 `props.content` 修改后能持久化；
...
- 组件包使用统计、安全删除、同 ID 替换/升级与失败隔离通过；已有图片可从素材库重复添加；编辑画布缩放/平移不改工程坐标；
...
新协议与渲染边界应同时运行 [Project V7 五路径渲染宿主基准](../examples/render-host-benchmark/README.md)。该基准覆盖原生节点、Runtime API 2 Phaser、Runtime API 2 DOM + 内联 Three.js、Component API 4 DOM 表格和 Component API 3 Phaser 兼容组件；自动化压力段固定执行 25 轮，共 100 次切页和 25 次重播，并核对宿主资源与外部请求没有累积。

最终原则只有六句：Project V7 JSON 是业务真相；效果第一；所有人工可见文字可编辑；简单互动优先使用可视化声明式映射；母版式通用元素和成品控制器优先使用 `globalLayer`；组件化只在高复用、参数化或独立维护真正有价值时使用。
